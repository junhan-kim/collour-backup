  ...
  <v-dialog v-model="display" :width="dialogWidth">
    <template v-slot:activator="{ on }">
      <v-text-field
        ...
        v-on="on"
        readonly
        :error-messages="datetimeErrorMessage"
      >
    ...
    <v-card>
      <v-card-text class="px-0 py-0">
        <v-tabs fixed-tabs v-model="activeTab">
          ...
          <v-tab key="timer" :disabled="dateSelected" class="mr-10">
            <slot name="timeIcon">
...
export default {
  ...
  methods: {
    ...
    okHandler() {
      this.resetPicker()
      if(this.$props.label=="시작일")
        this.$emit('startDateInput', this.selectedDatetime)
      else if(this.$props.label=="종료일")
        this.$emit('endDateInput', this.selectedDatetime)
    },
    clearHandler() {
      this.resetPicker()
      ...
      if(this.$props.label=="시작일")
        this.$emit('startDateInput', null)
      else if(this.$props.label=="종료일")
        this.$emit('endDateInput', null)
    },
    resetPicker() {
      this.display = false
      this.activeTab = 0
